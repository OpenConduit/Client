...
import { connectMcpServer, disconnectMcpServer, listAllTools, callTool, getMcpStatus, } from './mcp/client';
...
export function registerIpcHandlers() {
    ...
    ipcMain.on(IPC.TOOL_APPROVAL_RESPONSE, (_e, { toolId, approved }) => {
    ...
    // ─── MCP ─────────────────────────────────────────────────────────────────
    ipcMain.handle(IPC.MCP_CONNECT, async (_e, config) => {
        await connectMcpServer(config);
    });
    ipcMain.handle(IPC.MCP_DISCONNECT, async (_e, id) => {
        await disconnectMcpServer(id);
    });
    ipcMain.handle(IPC.MCP_LIST_TOOLS, async (_e, serverIds) => {
        return listAllTools(serverIds);
    });
    ipcMain.handle(IPC.MCP_STATUS, () => getMcpStatus());
    // ─── Models ──────────────────────────────────────────────────────────────
    ipcMain.handle(IPC.MODELS_LIST, async (_e, providerId) => {
        ...
        if (provider.type === 'openai' || provider.type === 'lmstudio') {
            try {
                ...
                const lmBaseUrl = provider.type === 'lmstudio'
        ...
        if (provider.type === 'anthropic') {
            ...
            try {
                ...
                const fetched = page.data.map((m) => m.id);
        ...
        if (provider.type === 'gemini') {
            ...
            try {
                ...
                for await (const m of await ai.models.list()) {
                    const name = m.name;
                    if (name)
                        fetched.push(name.replace('models/', ''));
                }
                if (fetched.length > 0) {
    ...
    ipcMain.handle(IPC.SETTINGS_EXPORT, async (_e, redact) => {
    ...
    ipcMain.handle(IPC.SETTINGS_IMPORT, async (_e) => {
        const win = BrowserWindow.getFocusedWindow();
    ...
    // ─── Routing Evaluation ────────────────────────────────────────────────────
    ipcMain.handle(IPC.ROUTING_EVALUATE, async (_e, params) => {
        const settings = getSettings();
    ...
    ipcMain.handle(IPC.UPDATE_CHECK, async () => {
        ...
        const channel = (getSettings().updateChannel ?? 'stable');
    ...
    // ─── Feedback Submit ──────────────────────────────────────────────────────
    ipcMain.handle(IPC.FEEDBACK_SUBMIT, async (_e, payload) => {
        const fullPayload = { ...payload, appVersion: app.getVersion(), platform: process.platform };
    ...
    // ─── Abort ───────────────────────────────────────────────────────────────
    ipcMain.on(IPC.CHAT_ABORT, (_e, conversationId) => {
        abortControllers.get(conversationId)?.abort();
    });
    // ─── Chat Send ───────────────────────────────────────────────────────────
    ipcMain.handle(IPC.CHAT_SEND, async (e, request) => {
        const wc = e.sender;
        const { conversationId, providerId, model, parameters, systemPrompt, enabledMcpServerIds } = request;
        ...
        (async () => {
            try {
                ...
                let messages = [...request.messages].filter((m) => m.role !== 'assistant' || !!(m.content || m.toolCalls?.length));
                ...
                if (enabledMcpServerIds.length > 0) {
                    ...
                    const serverConfigs = settings.mcpServers.filter((s) => enabledMcpServerIds.includes(s.id) && !currentStatus[s.id]);
                ...
                for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
                    if (abort.signal.aborted)
                        break;
                    const tools = enabledMcpServerIds.length > 0 ? await listAllTools(enabledMcpServerIds) : [];
                    const getStream = () => {
...
function requestApproval(wc, conversationId, messageId, toolCall) {
