...
function parseAndStripTasks(content) {
...
function parseAndStripQuestions(content) {
...
function buildSystemPrompt(basePrompt, settings) {
    const parts = [];
    if (basePrompt?.trim())
...
const compactingRequests = new Set();
...
function ensureListeners() {
    ...
    service.chat.onEnd((end) => {
        ...
        if (msg2)
            hookRegistry.runOnResponse(msg2);
        useUiStore.getState().setIsStreaming(false);
    });
    ...
    service.tools.onApprovalRequest((req) => {
...
export function useChat() {
    ...
    const { activeConversationId, isStreaming, setIsStreaming, isCompacting, pendingApprovals, removePendingApproval, } = useUiStore();
    useEffect(() => {
        ensureListeners();
    }, []);
    const activeConversation = conversations.find((c) => c.id === activeConversationId) ?? null;
    const sendMessage = useCallback(async (content, attachments) => {
        if (!activeConversationId || !settings || isStreaming)
            return;
        ...
        let request = {
            ...
            messages: freshConv.messages.filter((m) => m.role !== 'assistant' || !!(m.content || m.toolCalls?.length)),
            providerId,
            model,
            parameters: conv.parameters ?? settings.defaultParameters,
            systemPrompt: buildSystemPrompt(conv.systemPrompt, settings),
            enabledMcpServerIds: freshConv.activeMcpServerIds ??
    ...
    const compactContext = useCallback(async () => {
        if (!activeConversationId || !settings || isStreaming || isCompacting)
            return;
    ...
    const approveToolCall = useCallback((toolId, approved) => {
    ...
    const sendAnswers = useCallback((questions, answers) => {
