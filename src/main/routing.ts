...
export async function evaluateRouting(params: {
  ...
  const complexity = Math.max(1, Math.min(3, Math.round(classification.complexity))) as 1 | 2 | 3;
  const taskType: RoutingTaskType = classification.taskType ?? 'general';
